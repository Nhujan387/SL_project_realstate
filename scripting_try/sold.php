                   ...
                   <?php
                       ...
                       FROM `sale` as s inner join user as u on s.user_id=u.user_id WHERE s.s_id = '$_REQUEST[s_id]' ";
                        ...
                        <div style="width:80%;margin:0 auto;height:37vh;padding:10px;">
                            ...
                            <form method="post">
                                <input type="text" value="1" name="stat" id="stat" hidden>
                                <input type="submit" class="consold" value="Confirm Sold" />
                            </form>
                        </div> 
                    </div>
               </div>
...
<?php
    if($_POST){
        ...
        $update = "UPDATE `sale` SET `status`='$status' where s_id = '$_REQUEST[s_id]'";

        if($update){
            ?><script>alert('Congratulation!!');location.replace("sell.php");;</script><?php
        }
