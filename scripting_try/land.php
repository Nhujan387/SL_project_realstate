<?php
    ...
    $fetch = "SELECT * FROM `sale` WHERE status='0'";
    ...
    if($totalResult>0){
        ...
        $sql="SELECT * FROM `sale` LIMIT  $currentPage,$searchlimit";
...
<html lang="en">
    <head>
        ...
        <title>Residence:House Buy</title>
    ...
    <body>
        ...
        <div class="forsale">
            ...
            <?php 
                ...
                FROM `sale` as s inner join user as u on s.user_id=u.user_id `sale` WHERE property_type='land' AND status='0' AND district='$location' ORDER BY `Postdate` DESC ";
                 $queryfetch = mysqli_query($conn,$fetch);
                ...
                <?php } }else { 
                    $fetch = "SELECT s.s_id, s.property_type, s.Postdate, s.province, s.district, s.address, s.ropani, s.aana, s.paisa, s.dam,
                    s.bedroom_number, s.toilet_number, s.floor, s.amount, s.current_address, s.phone, s.image, s.user_id, s.status , u.name 
                   FROM `sale` as s inner join user as u on s.user_id=u.user_id WHERE property_type='land' AND status='0' ORDER BY `Postdate` DESC";
                    $queryfetch = mysqli_query($conn,$fetch);
                    while($result = mysqli_fetch_assoc($queryfetch)){
